...
cpoMultiplex = function(cpos, selected.cpo = NULL, id = NULL, export = "export.default",
  ...
  rl = makeCPOExtendedTrafo("multiplex", par.set = paramset, par.vals = pv, dataformat = "task",
    properties.data = pr$data, properties.adding = pr$adding,
    properties.needed = pr$needed, properties.target = pr$target,
    cpo.trafo = function(data, target, selected.cpo, ...) {
...
#'   Parameters (additionally to the exported CPOs) of the CPO
...
#'   Can be one or many of the same values as \dQuote{properties.data} for Feature Operation CPOs, and one or many of the same values as \dQuote{properties.data.target}
...
#'   this should be \dQuote{missings}. This must be a subset of \dQuote{properties.data} or \dQuote{properties.data.target}. Default is
#'   \code{character(0)}.
#' @param properties.needed [\code{character}]\cr
#'   Can be one or many of the same values as \dQuote{properties.data} for Feature Operation CPOs,
...
cpoCase = function(par.set = NULL, par.vals = list(), export = list(),
  ...
  rl = makeCPOExtendedTrafo("case", par.set = c(paramset.pass.on, paramset.others), par.vals = c(pv.pass.on, pv.others),
    dataformat = "task", dataformat.factor.with.ordered = FALSE, properties.data = properties.data, properties.adding = properties.adding,
    ...
    cpo.trafo = function(data, target, ...) {
      ...
      tin = prepareTrafoInput(data, dataformat, c(properties.data, properties.target, cpo.predict.properties), fullaffect, FALSE, "case")
...
collectProperties = function(constructed, do.intersect = FALSE) {
  ...
  all.allprops = lapply(constructed, function(x) setdiff(getCPOProperties(x)$handling, c(cpo.dataproperties, cpo.predict.properties)))

  if (do.intersect) {
  list(
      data= Reduce(intersect, extractSubList(allprops, "handling", simplify = FALSE), cpo.dataproperties),
      needed = Reduce(union, extractSubList(allprops, "needed", simplify = FALSE), character(0)),
      adding = Reduce(intersect, extractSubList(allprops, "adding", simplify = FALSE),
        if (!length(oc) || length(oc) > length(constructed)) character(0) else cpo.dataproperties),
      target = Reduce(intersect, all.allprops, c(cpo.tasktypes, cpo.targetproperties)))
  } else {
    list(
        data = Reduce(union, extractSubList(allprops, "handling", simplify = FALSE)),
        needed = Reduce(intersect, extractSubList(allprops, "needed", simplify = FALSE)),
        adding = Reduce(union, extractSubList(allprops, "adding", simplify = FALSE)),
        target = Reduce(union, all.allprops))
  }
}
