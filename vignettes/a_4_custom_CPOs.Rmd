...
```{r}
retr.df = getTaskData(test.task, target.extra = TRUE)$data %>>% retrafo(trafo)
...
```{r}
ext.model = train("classif.svm", trafo)
...
As an aside, the `Learner` enhanced by `xmpMetaLearn` seems to perform marginally better than `"classif.logreg"` on its own for large enough data sets (here resampled with output suppressed).
```{r, echo = FALSE}
...
learners = list(
    ...
    cpo = xmpMetaLearn(makeLearner("classif.logreg")) %>>% makeLearner("classif.svm")
)
