...
```{r}
iris.demo = iris[c(1, 2, 51, 52, 102), ]
...
```{r}
# first create bins, then as.numeric() all columns to get 1 or 2 for below & above median
quantilenum = cpoQuantileBinNumerics() %>>% cpoAsNumeric()
...
```{r}
quantilenum.restricted = cpoQuantileBinNumerics() %>>%
