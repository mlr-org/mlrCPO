...
Because we only do tuning in low dimensions, it is sensible to use grid tuning. For better visualisation, we choose a relatively large resolution.
...
ps = makeParamSet(makeNumericParam("pca.rank", lower = 0, upper = 9))
rcv10 = makeResampleDesc("RepCV", folds = 10, reps = 10)
tuneres = tuneParams(tune.learner, titanic.task, rcv10, par.set = ps, control = ctrl, show.info = TRUE)
tuneres
```

While a bit noisy, the result indicates that there seems to be a "sweet spot" near 50% of features.
...
Unfortunately, the performance does not seem to be better than the performance on the original data.

```{r}
repcv(preproc.learner, titanic.task, show.info = FALSE)
repcv(setHyperPars(tune.learner, kruskal.test.perc = 0.2), titanic.task, show.info = FALSE)
repcv(preproc.pipeline %>>% makeLearner("classif.randomForest"), titanic.task, show.info = FALSE)
