...
tuneres
```
The result seems to indicate that the performance is similar for the first five added columns, after which it degrades. Note that all tuning result graphs shown were created using 10-fold repeated crossvalidation with 10 iterations, which reduces noise but takes a lot longer to run than the simple 10-fold crossvalidation indicated above.
...
The difference seems to be minute, the difference between ICA and PCA is smaller than the resampling error.
